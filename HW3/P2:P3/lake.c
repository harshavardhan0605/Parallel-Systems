...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
 ...
 // memcpy(uc, u1, sizeof(double) * n * n); 
  #pragma omp parallel for schedule(static) private(i)       // Instead of memcpy for Initial Config, Parallelizing a for loop for
  for(i=0;i<n*n;i++)                                                                                            //        optimization
  {
    uo[i]= u0[i];
    uc[i] = u1[i];
  }
  ...
  #pragma acc data copy(uc[:n*n]) copyin(uo[:n*n], un[:n*n],pebbles[:n*n])           //Data Copying to Device for OpenACC
  while(1)
  {

    /* run a central finite differencing scheme to solve
     * the wave equation in 2D */
    #pragma omp parallel for schedule(static) private(i,j,idx)                     // Outer Loop OpenMP
    #pragma acc parallel loop present(uc[:n*n], uo[:n*n], un[:n*n],pebbles[:n*n])   // OpenACC Optimization
    for( i = 0; i < n; i++)
    {
     // #pragma omp parallel for schedule(static) private(idx)                      //Used for Inner loop Parallelization   
      for( j = 0; j < n; j++)
      {
        idx = j + i * n;
   ...
   double *temp;                                 // Instead of memcpy shifting Pointers for Optimization
   temp = uo;
   uo= uc;
   uc = un;
   un = temp;
...
void init(double *u, double *pebbles, int n)
{
  ...
  #pragma omp parallel for schedule(static) private(i,j,idx) 
  for(i = 0; i < n ; i++)
  {
    for(j = 0; j < n ; j++)
    {
